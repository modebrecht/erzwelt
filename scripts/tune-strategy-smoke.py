...
# Product-line staffing is a deliberate investment; allow the bot to use more of\n# its already-reserved cash instead of refusing a sensible hire at CHF 150k.
mine_hire="        if not hire_needed(target-cur):return False\n        sheet('mine',id)\n"
if s.count(mine_hire)!=1: raise SystemExit(f'mine hire guard count {s.count(mine_hire)}')
s=s.replace(mine_hire,"        if not hire_needed(target-cur,100000):return False\n        sheet('mine',id)\n")
